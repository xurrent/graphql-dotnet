...
namespace Xurrent.GraphQL.Tests
{
    ...
    public class BulkTest
    {
        ...
        public void Export()
        {
            string csvPath = Path.Combine(Path.GetTempPath(), "xurrent_cis.csv");
            string zipPath = Path.Combine(Path.GetTempPath(), "xurrent_export.zip");
        ...
        public void Import()
        {
            string csvPath = Path.Combine(Path.GetTempPath(), "xurrent_cis_import.csv");

            File.WriteAllText(csvPath, $"\"ID\",Source,Source ID\n20,Xurrent.GraphQL,\"{DateTime.Now:o}\"\n", new UTF8Encoding(false));
