...
namespace Xurrent.GraphQL.Helpers
{
    ...
    internal sealed class ExecutionQueryBuilder
    {
        ...
        /// <exception cref="XurrentInvalidFieldNameException"></exception>
        internal static ExecutionQuery BuildQuery(IQuery query, int defaultItemsPerRequest)
        {
            if (string.IsNullOrEmpty(query.FieldName))
                throw new XurrentInvalidFieldNameException($"{nameof(query)} first field is not a root query field.");
            ...
            else if (executionQuery.IsConnection)
            {
                ...
                if (executionQuery.OnlyQueryIdentifier)
                {
                    if (!executionQuery.HasIdentifier)
                        throw new XurrentException($"Pagination not possible, {executionQuery.FieldName} has no identifier.");
                    builder.Append("id ");
                }
                else
                {
        ...
        private static ExecutionQuery BuildQuery(ExecutionQuery query)
        {
            foreach (ExecutionQueryField field in query.Fields)
            {
                if (field.IsSelected)
                {
                    ...
                    {
                        if (query.Queries.FirstOrDefault(x => x.FieldName.Equals(field.Name, StringComparison.Ordinal)) is null)
                        {
                    ...
                    else if (field.IsCustomFields)
                    {
                        ...
                        {
                            throw new XurrentException($"{field.Name} did not implement {nameof(KeyedCollection<string, CustomField>)}.");
                        }
                        field.IsSelected = false;
                    }
        ...
        private static List<ExecutionQueryField> GetExecutionQueryFields(Type dataType, ImmutableHashSet<string>? selectedFields)
        {
            ...
            if (selectedFields != null)
            {
                foreach (string selectedField in selectedFields)
                {
                    ...
                    {
                        throw new XurrentInvalidFieldNameException(selectedField);
                    }
                }
            }
        ...
        /// <exception cref="XurrentFilterException"></exception>
        internal static string BuildDateTimeFilter(string field, FilterOperator filterOperator, params DateTime?[] values)
        {
            ...
            throw new XurrentFilterException($"Unsupported date time filter operator, use " +
        ...
        internal static string BuildBooleanFilter(string field, FilterOperator filterOperator, bool value)
        {
            return filterOperator switch
            {
                ...
                _ => throw new XurrentFilterException($"Unsupported boolean filter operator. Supported operators are {nameof(FilterOperator.Equals)}, {nameof(FilterOperator.NotEquals)}, {nameof(FilterOperator.In)}, or {nameof(FilterOperator.NotIn)}."),
            };
        }
        ...
        internal static string BuildCustomFilter(string name, FilterOperator filterOperator, params string?[] values)
        {
            ...
            {
                throw new XurrentFilterException($"Unsupported custom filter operator. Supported operators include " +
        ...
        internal static string BuildStringFilter(string field, FilterOperator filterOperator)
        {
            ...
            throw new XurrentFilterException($"Unsupported string filter operator. Please use either {nameof(FilterOperator.Present)} or {nameof(FilterOperator.Empty)} when no values are provided.");
        }
        ...
        internal static string BuildStringFilter(string field, FilterOperator filterOperator, params string?[] values)
        {
            if (field == "id" && values.Any(value => value == null))
                throw new XurrentFilterException("All values must be non-null when the field ID is specified for filtering.");
            ...
            {
                throw new XurrentFilterException($"Unsupported string filter operator. Supported operators include " +
                    $"{nameof(FilterOperator.Equals)}, {nameof(FilterOperator.NotEquals)}, {nameof(FilterOperator.In)}, {nameof(FilterOperator.NotIn)}, " +
                    $"{nameof(FilterOperator.Present)}, and {nameof(FilterOperator.Empty)}.");
            }
            return builder.ToString();
