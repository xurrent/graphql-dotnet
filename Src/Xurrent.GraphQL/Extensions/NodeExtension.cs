...
namespace Xurrent.GraphQL
{
    ...
    {
        /// <summary>
        /// Extracts the Xurrent instance identifier from a GraphQL identifier.
        /// </summary>
        /// <param name="node">An <see cref="Node"/> object containing the GraphQL identifier.</param>
        /// <returns>The Xurrent instance identifier extracted from the GraphQL identifier.</returns>
        /// <exception cref="XurrentException">Thrown when the identifier is not supported or invalid.</exception>
        ...
        private static string[] FromBase64(string identifier)
        {
            string[] retval = (identifier.Length % 4) switch
            {
                ...
                _ => throw new XurrentException("Unsupported GraphQL or nodeID value.")
            };

            if (retval.Length != 3)
                throw new XurrentException("Unsupported GraphQL or nodeID value.");
