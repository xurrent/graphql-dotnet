...
namespace Xurrent.GraphQL
{
        ...
        where TOrderBy : Enum
    {
        ...
        protected Query(string fieldName, Type dataType, bool isConnection)
        {
            ...
            this.dataType = typeof(Node).IsAssignableFrom(dataType) ? dataType : throw new XurrentException($"{nameof(dataType)} does not implement {nameof(Node)}.");
        ...
        {
            this.fieldName = fieldName;
            this.id = !string.IsNullOrEmpty(id) ? id : throw new XurrentException($"The {nameof(id)} cannot be null or empty.");
            this.dataType = typeof(Node).IsAssignableFrom(dataType) ? dataType : throw new XurrentException($"{nameof(dataType)} does not implement {nameof(Node)}.");
            this.isConnection = isConnection;
        }
        ...
        public TEntity OrderBy(string value, OrderBySortOrder orderBySortOrder)
        {
            ...
            return this as TEntity ?? throw new ArgumentException(nameof(TEntity));
        }
